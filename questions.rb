...
def array_sort_by_last_letter_of_word(array)
  array.sort {|first, last| first[-1] <=> last[-1]}
end
...
def number_of_elements_that_are_palindromes(array)
  array.reject {|word| word != word.reverse}.count
end

# return the shortest word in an array
def shortest_word_in_array(array)
  array.sort {|a,b| a.length <=> b.length}.fetch(0)
end

# return the longest word in an array
def longest_word_in_array(array)
  array.sort {|a,b| b.length <=> a.length}.fetch(0)
end
...
def get_all_letters_in_array_of_words(array)
  array.map {|word| word.chars }.flatten.uniq.sort
end
...
def is_a_3_dot_range?(range)
  (range.first .. range.last).size != range.size
end
...
def count_words_of_each_length_in_a_file(file_path)
  file = File.open(file_path).to_a
  file[0].gsub(/\W/,' ').split(' ').inject(Hash.new{0}) do |times, word|
    times[word.length] +=1
    times
  end
end
